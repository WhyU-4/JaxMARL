...
class SimpleTag3v1MPE(SimpleMPE):
    """
    3v1 Pursuit-Evasion Environment
    
    This environment features 3 pursuers (adversaries) chasing 1 evader (good agent).
    The pursuers aim to catch the evader while the evader tries to avoid being caught.
    
    Agents:
    - 3 pursuers (adversary_0, adversary_1, adversary_2): slower but collaborative
    - 1 evader (agent_0): faster and more agile
    
    Observations include:
    - For pursuers: velocity, position, landmark positions, other agent positions and velocities
    - For evader: velocity, position, landmark positions, other agent positions
    
    Rewards:
    - Pursuers: positive reward for catching the evader
    - Evader: negative reward for being caught, negative reward for going out of bounds
    """
        ...
        def _good(aidx):
            return jnp.concatenate(
                [
                    ...
                    landmark_pos[aidx].flatten(),  # 2*num_landmarks
                    other_pos[aidx].flatten(),  # 2*(num_agents-1)
                ]
            )
        ...
        def _adversary(aidx):
            return jnp.concatenate(
                [
                    ...
                    landmark_pos[aidx].flatten(),  # 2*num_landmarks
                    other_pos[aidx].flatten(),  # 2*(num_agents-1)
                    other_vel[aidx, -1:].flatten(),  # 2 (evader velocity)
                ]
            )
